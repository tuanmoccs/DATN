<template>
  <div class="bg-white border border-gray-200 rounded-lg p-4">
    ...
    <p class="text-xs text-gray-500 mb-3">
      Upload tài liệu để AI có ngữ cảnh gợi ý nội dung chính xác hơn.
    </p>
    ...
    <div @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="handleDrop"
      ...
      <input ref="fileInput" type="file" class="hidden" accept=".pdf,.doc,.docx,.txt" @change="handleFileSelect" />
      <i class="fas fa-cloud-upload-alt text-2xl text-gray-400 mb-2"></i>
      <p class="text-sm text-gray-500">Kéo thả file hoặc click để chọn</p>
      <p class="text-xs text-gray-400 mt-1">PDF, DOC, DOCX, TXT (tối đa 20MB)</p>
    </div>
    ...
    <div class="mt-4 border-t border-gray-100 pt-3">
      ...
      <div v-if="showTextInput" class="mt-2">
        <textarea v-model="referenceText" rows="5" placeholder="Dán nội dung tài liệu tham khảo vào đây..."
          class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-y"></textarea>
        <button @click="submitText" :disabled="!referenceText.trim() || isUploading"
...
async function uploadFile(file) {
  ...
  if (file.size > maxSize) {
    uploadError.value = 'File quá lớn. Tối đa 20MB.'
    return
  }
  ...
  if (!allowedTypes.includes(file.type)) {
    uploadError.value = 'Định dạng không hỗ trợ. Chỉ chấp nhận PDF, DOC, DOCX, TXT.'
    return
  }
  ...
  try {
    ...
    uploadError.value = err?.response?.data?.message || 'Lỗi khi upload tài liệu'
  } finally {
...
async function submitText() {
  ...
  try {
    ...
    uploadError.value = err?.response?.data?.message || 'Lỗi khi xử lý nội dung'
  } finally {
    isUploading.value = false
  }
}
